...
def test_llm_dashscope(functions, stream, delta_stream):
    ...
    # setting
    llm_cfg = {'model': 'qwen-plus', 'model_server': 'dashscope', "api_key": "sk-22a3f18de8c840d79d3d16f821c9a160"}
...
def test_llm_dashscope(stream):
    ...
    # Chat with text llm
    llm = get_chat_model(llm_cfg)
    messages = [Message('user', '请介绍一下你自己')]
    response = llm.chat(messages=messages, stream=stream)
    if stream:
        response = list(response)[-1]
    print(response)
    assert isinstance(response[-1]['content'], str)
